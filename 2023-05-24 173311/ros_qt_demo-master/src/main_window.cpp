    ...
    , qnode(argc,argv)
{
    ...
    // ensure the first tab is showing - qt-designer should have this already hardwired, but often loses it (settings?).
    ...
    **********************/
    ...
    /*********************
    ** Server for Windows QT client to connect
    **********************/
    ...
    /*********************
    ** Server for Android App client to connect
    **********************/
    ...
    */
    ...
    // 绑定到指定的端口上
    bool result=udpSocket->bind(QHostAddress("127.0.0.1"),port);
    if(!result)
    {
        qDebug()<<"UDP error!";
    }
    ...
    music_player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "06_music.mp3"));
    ...
    cout << "cloud_ip:" << config["cloud_ip"].as<string>() << endl;
    cout << "cloud_port:" << config["cloud_port"].as<int>() << endl;
    ui.line_IP->setText(QString::fromStdString(config["cloud_ip"].as<string>()));
    ui.line_port->setText(QString::fromStdString(config["cloud_port"].as<string>()));

    prompt_voice_path = QString::fromStdString(config["prompt_voice_path"].as<string>());

    // Print all video name
    YAML::Node config2 = YAML::LoadFile(yaml_file_path);
    for(auto it = config2["video_name"].begin(); it != config2["video_name"].end(); it++)
    {
        std::cout << "item: " << (*it).as<string>() << std::endl;//也可以用迭代器访问
    }

    // car_cloud_connect(); // Connect to platform
    // ui.connect_cloud->click();  // Connect to platform
}
...
void MainWindow::slot_send_current_car_point()
{
    ...
    // qDebug() << "RoboPose_data_cloud: " << RoboPose_data_cloud;
    if(!RoboPose_data_cloud.isEmpty())
...
void MainWindow::handle_timeout_send_multi_navi_srart()
{
    ...
    qDebug() << "start multi";
    player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "03_start_multi_navigation.mp3"));
    player->play();
...
void MainWindow::handle_timeout_send_navi_point()
{
    ...
    player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "01_set_goal_point.mp3"));
    player->play();
...
void MainWindow::handle_timeout_send_car_return()
{
    ...
    player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "02_start_return.mp3"));
    player->play();
...
void MainWindow::handle_timeout_send_fault()
{
    ...
    if(laser_fault_count > fault_count_threshold)
    {
        ...
        player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "laser_fault.mp3"));
        player->play();
    }
    ...
    if(imu_fault_count > fault_count_threshold)
    {
        ...
        sendcommendwithdata(send_car_fault, robot_ID, fault_code);
        player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "imu_fault.mp3"));
        player->play();
    }
    */
}
...
void MainWindow::slot_get_car_move_status(int status)
{
    ...
    //        case 0:
    //        {
    //            player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "07_first_point.mp3"));
    //            player->play();
    //            break;
    //        }
    //        case 1:
    //        {
    //            player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "08_second_point.mp3"));
    //            player->play();
    //            break;
    //        }
    //        case 2:
    //        {
    //            player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "09_third_point.mp3"));
    //            player->play();
    //            break;
    //        }
    //        default:
...
void MainWindow::handle_current_point_signal_(double pos_x_, double pos_y_, double pos_z_, double pos_w_)
{
    ...
    // send current pose to thread_trackPath
    emit current_pose_(pos_x_, pos_y_, pos_z_, pos_w_);
...
//             << " w = " << pos_w_;
    ...
    if(set_return_flag == 1)
    {
        qDebug() << " x = " << pos_x_
                 << " y = " << pos_y_
                 << " z = " << pos_z_
                 << " w = " << pos_w_;
        qnode.sendreturn_pos(pos_x_,pos_y_,pos_z_,pos_w_);
        set_return_flag = 0;
    }

    if(record_path_flag)
    {
        if(points_of_path.isEmpty())
        {
            points_of_path.push_back(QPointF(pos_x_, pos_y_));
            qDebug() << points_of_path;
        }
        else
        {
            double instance_of_two_points_pow2 =
                    qPow(((*points_of_path.rbegin()).x() - pos_x_), 2)
                    + qPow(((*points_of_path.rbegin()).y() - pos_y_), 2);

            if(sqrt(instance_of_two_points_pow2) > interval_of_path_points)
            {
                points_of_path.push_back(QPointF(pos_x_, pos_y_));
                // qDebug() << points_of_path;
            }
...
void MainWindow::slot_batteryState(float p)//获取电池电压
{
    ...
    // qDebug() << "battery_str: " << battery_str;
}
...
void MainWindow::cloud_handledata(uint8_t commend,uint32_t roboid, QByteArray r_data)
{
    ...
    */
    ...
    case connect_to_cloud_plat: //0x40
    {
        ...
        // sometimes can't read amcl_pose, ERROR: RemoteHostClosedError
        timer_send_current_car_point->start(12345);
        ...
        if(!setstartstatue)
        {
            ...
            emit sendsetreturnpossignal();
            setstartstatue = true;
            qDebug() << "set start point to return point success";
        }
    ...
    case return_pos_signal: // 0x0D
    {
        ...
        player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "04_set_return_point.mp3"));
        player->play();
    }break;
    ...
    case multipos_signal:   //0x20
    {
        ...
        // hanglemulti_pos(r_data);
        player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "05_set_multi_navigation_points.mp3"));
        player->play();
    }break;
    ...
    case receive_new_video:  // 0x52
    {
        handle_receive_new_video(r_data);
        sendcommendwithdata(receive_new_video, robot_ID, empty_ba);
    }break;

    case record_path:  // 0x53
    {
        handle_record_path(r_data);
        sendcommendwithdata(record_path, robot_ID, empty_ba);
    }break;

    case start_track_path:  // 0x55
    {
        handle_track_path(r_data);
        sendcommendwithdata(start_track_path, robot_ID, empty_ba);
    }break;
...
void MainWindow::handle_play_audio(QByteArray r_data)
{
    ...
    case 0x00:
    {
        player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "00_hello.mp3"));
        player->play();
        break;
    }
    default:break;
...
void MainWindow::handle_receive_new_video(QByteArray r_data)
{
    // filename
    uint8_t r_data_at3 = (uint8_t)r_data.at(3);//现将二进制数据流转为uint_8型，否则数据转32位会出错
    uint8_t r_data_at2 = (uint8_t)r_data.at(2);
    ...
    // save current video name
    YAML::Node config2 = YAML::LoadFile(yaml_file_path);
    ofstream fout2(yaml_file_path);
    config2["video_name_current"] = file_name_str.toStdString();
    fout2 << config2;
    fout2.close();
    ...
    YAML::Node config = YAML::LoadFile(yaml_file_path);
    file_path_ = QString::fromStdString(config["video_path"].as<string>());
    file_name_ = file_name_str;

    for(auto it = config["video_name"].begin(); it != config["video_name"].end(); it++)
    {
        std::cout << "item: " << (*it).as<string>() << std::endl;//也可以用迭代器访问
        if(QString::fromStdString((*it).as<string>()) == file_name_str)
        {
            qDebug() << "same video, don't need download";
            return;
        }
    }

    qDebug() << "new video ... ";
    ofstream fout(yaml_file_path);
    config["video_name"].push_back(file_name_str.toStdString());
    fout << config;
    fout.close();

    // video_download_flag = 1;
    // download video
    startRequest(url_str_);

    /*
    if(QString::fromStdString(config["video_name"].as<string>()) == file_name_str)
    {
        qDebug() << "same video, don't need download";
    }
    else
    {
        qDebug() << "new video ... ";
        ofstream fout(yaml_file_path);
        config["video_name"] = file_name_str.toStdString();
        fout << config;
        fout.close();
...
{
    YAML::Node config = YAML::LoadFile(yaml_file_path);
    // cout << "previous video_name:" << config["video_name"].as<string>() << endl;
    thread_play_video->file_url =
            QString::fromStdString(config["video_path"].as<string>())
           + QString::fromStdString(config["video_name_current"].as<string>());
        // + QString::fromStdString(config["video_name"].as<string>());

    // while(video_download_flag);

    qDebug() << "play this video:" << thread_play_video->file_url;
    qDebug() << "主线程id：" << QThread::currentThreadId();
    thread_play_video->start(); // 启动子线程
...
void MainWindow::handle_record_path(QByteArray r_data)
{
    switch (r_data.at(0))
    {
    case 0x00:
    {
        // stop record path
        record_path_flag = 0;
        // do something to : QPolygonF points_of_path
        qDebug() << "stop record path, points_of_path: ";
        qDebug() << points_of_path;
        play_voice("13_stop_record_path.mp3");

        handle_points_of_path();
        break;
    }
    case 0x01:
    {
        ...
        // start record path
        qDebug() << "start record path";
        play_voice("12_start_record_path.mp3");
...
void MainWindow::handle_points_of_path()
{
    if( points_of_path.size() < 2 )
    {
        qDebug() << "points of path less than 2";
        return;
    }
    ...
    for(int i = 0; i < points_of_path.size() ;i++)
    {
        //if(i = points_of_path.size())
        QPointF startmultipos = points_of_path.at(i);
        QPointF lastmultipos = points_of_path.at(i + 1);
    ...
    gui_path.poses.resize(pose_list.poses.size());

    gui_path.header.frame_id = "map";
    gui_path.header.stamp = ros::Time::now();

    // Extract the plan in world co-ordinates, we assume the path is all in the same frame
    for (unsigned int i = 0; i < pose_list.poses.size(); i++) {
        gui_path.poses[i].pose = pose_list.poses[i];
    }

    qDebug() << "gui_path.poses.size(): " << gui_path.poses.size();
    qnode.handle_pub_global_path(gui_path);

    YAML::Node global_path;
    std::vector<double> v_;
    v_.clear();
    for (unsigned int i = 0; i < pose_list.poses.size(); i++) {
        v_.push_back(pose_list.poses[i].position.x);
        v_.push_back(pose_list.poses[i].position.y);
        v_.push_back(pose_list.poses[i].orientation.z);
        v_.push_back(pose_list.poses[i].orientation.w);
        global_path["poses"].push_back(v_);
        v_.clear();
        // gui_path.poses[i].pose = pose_list.poses[i];
    }

    ofstream out(yaml_global_path, ios::trunc | ios::out);
    out << global_path << endl;
    out.close();

    // test navigation
/*     YAML::Node config2 = YAML::LoadFile("/home/smarcorobot/catkin_zy/src/ros_qt_demo-master/global_path.yaml");
     vector<vector<double>> poses = config2["poses"].as<vector<vector<double>>>();
     for(int i = 0; i < poses.size(); i++)
     {
         for(int j = 0; j < poses.at(i).size(); j++)
         {
             cout << poses.at(i).at(j) << endl;
         }
     }
...
void MainWindow::handle_track_path(QByteArray r_data)
{
    switch (r_data.at(0))
    {
    case 0x00:
    {
        // stop track path
        delete my_thread_trackPath;
        my_thread_trackPath = nullptr;
    }
    case 0x01:
    {
        // start track path
        if(my_thread_trackPath != nullptr){
            delete my_thread_trackPath;
            my_thread_trackPath = nullptr;
        }
        qDebug() << "new thread_trackPath()";
        my_thread_trackPath = new thread_trackPath(yaml_global_path);
        connect(my_thread_trackPath,
                SIGNAL(SET_GOAL_(QString, double, double, double, double)),
                this,
                SLOT(handle_set_goal_(QString, double, double, double, double)));

        // send current pose
        connect(this,
                SIGNAL(current_pose_(double, double, double, double)),
                my_thread_trackPath,
                SLOT(handle_current_pose_(double, double, double, double)));

        connect(my_thread_trackPath,
                SIGNAL(PUB_GLOBAL_PATH()),
                this,
                SLOT(handle_pub_global_path()));


        // qDebug() << "start my_thread_trackPath";
        my_thread_trackPath->start();
    }
    }
}

void MainWindow::handle_set_goal_(QString frame, double x, double y, double z, double w)
{
//    qDebug() << "function, handle_set_goal_";
//    qDebug() << "goal is "
//             << "x = " << x
//             << "y = " << y
//             << "z = " << z
//             << "w = " << w;
    ...
    // qnode.set_datarequest(8);//发送停止小车命令
    // qnode.set_goal("map",startgoalpos.x(),startgoalpos.y(),goal_pos_quat.z,goal_pos_quat.w);
    qnode.set_goal("map", x, y, z, w);
}

void MainWindow::handle_pub_global_path()
{
    qDebug() << "function handle_pub_global_path()";
    std::vector<geometry_msgs::PoseStamped>* path_;
    path_ = new std::vector<geometry_msgs::PoseStamped>();

    YAML::Node global_path = YAML::LoadFile(yaml_global_path);
    ...
    for(int i = 0; i < poses.size(); i++)
    {
        ...
        qDebug() << " set goal, "
                 << " pose_x: " << poses.at(i).at(0)
                 << " pose_y: " << poses.at(i).at(1)
                 << " pose_z: " << poses.at(i).at(2)
                 << " pose_w: " << poses.at(i).at(3)
                 << " success!";
    }

    //create a message for the plan
    nav_msgs::Path gui_path;
    gui_path.poses.resize(path_->size());
    ...
    for (unsigned int i = 0; i < path_->size(); i++) {
        gui_path.poses[i].pose = path_->at(i).pose;
    }
...
void MainWindow::play_voice(QString voice_file_name)
{
    player->setMedia(QUrl::fromLocalFile(prompt_voice_path + voice_file_name));
    player->play();
}
...
void class1_ros_qt_demo::MainWindow::on_pushButton_play_music_clicked()
{
    ...
    // music_player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "1.mp3"));
...
void class1_ros_qt_demo::MainWindow::replyFinished()
{
    ...
    if (statusCode >= 200 && statusCode <300){
        ...
        QString file_name = file_path_ + file_name_;
...
void class1_ros_qt_demo::MainWindow::reply_readyRead()
{
    ...
    if(video_download_start_flag == 0)
    {
        player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "10_start_download_video.mp3"));
        player->play();
        video_download_start_flag = 1;
    }
}
...
void class1_ros_qt_demo::MainWindow::reply_downloadProgress(qint64 bytesRead, qint64 bytesTotal)
{
    ...
    {
        player->setMedia(QUrl::fromLocalFile(prompt_voice_path + "11_finish_download_video.mp3"));
        player->play();
        video_download_start_flag = 0;
    }
}

}  // namespace class1_ros_qt_demo

void class1_ros_qt_demo::MainWindow::on_pushButton_pub_path_clicked()
{
    geometry_msgs::Pose pose;
    pose.position.x = 10;
    pose.position.y = 20;
